...
/**
 * @brief calculate the chains created of an edge with given direction
 * 
 * @param e: The new edge that is put on board
 * @param dir: the direction to search at
 * 
 * @return pair<int, Edge*>: 
 *         int: number of chains created
 *         Edge*: the last edge of chain
*/
...
/**
 * @brief calculate the total number of chains created by an edge
 * 
 * @param e: The new edge that is put on board
 * 
 * @return int: 
 *         the total number of chains created by Edge* e
*/
int Board::totalChainsCreated(Edge* e) {
    ...
    if (e->type == VERTICAL) {
        ...
        //check if it is a closed cycle of chain
        int score = doesCompleteSquare(resultLeft.second);
        if (score == 0 || count == 0 || (count == 1 && score != 0)) 
            count += chainsCreated(e, RIGHT).first;
    }
    else {
        pair<int, Edge*> resultUp = chainsCreated(e, UP);
        count = resultUp.first;
        int score = doesCompleteSquare(resultUp.second);
        if (score == 0 || count == 0 || (count == 1 && score != 0)) 
