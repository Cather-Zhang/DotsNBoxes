...
*/
/*
pair<int, vector<GameStatus*>> Minimax::minimaxAlphaBeta(GameStatus* currentState, int depth, int alpha, int beta, bool maximizingPlayer, auto endtime) {
    ...
    if (maximizingPlayer) {
        ...
        for (GameStatus* child: children) {
            ...
            childResult = child->doesPrevMoveScore ? 
                // If the child node is a score, we keep recursive searching the child
                // without decreasing depth for evaluating chain scoring
                minimaxAlphaBeta(child, depth, alpha, beta, true, endtime) :
        ...
        for (GameStatus* child: children) {
            ...
            childResult = child->doesPrevMoveScore ? 
                minimaxAlphaBeta(child, depth, alpha, beta, false, endtime) :
...
Edge* Minimax::getNextMove(GameStatus* gs) {
    ...
    else paths = iterativeDeepeningMinimax(gs, 8);
